...
func main() {
	...
	var (
		...
		minAccountBalance      = os.Getenv(Createkey("min-account-balance"))      // 0.001, "Minimum balance in ETH for an account to get its messages relayed. Defaults to 0.001 ETH")
		maxDataSize            = os.Getenv(Createkey("max-data-size"))            // 1024 "Size limit for message data. Defaults to 1024 bytes")
		verifyMessageSignature = os.Getenv(Createkey("verify-msg-sig"))           //false "Whether to service-side verify the signature on each relayed message. Defaults to false")
	...
	// Build a config.
	idleTimeoutTimeInt, _ := strconv.Atoi(idleTimeoutTime)
	portInt, _ := strconv.Atoi(port)
	metricsPortInt, _ := strconv.Atoi(metricsPort)
	idleDisconnectIntervalInt, _ := strconv.Atoi(idleDisconnectInterval)
	messageDriftTimeInt, _ := strconv.Atoi(messageDriftTime)
	maxDataSizeInt, _ := strconv.Atoi(maxDataSize)
	messageRateLimitInt, _ := strconv.Atoi(messageRateLimit)
	minAccountBalanceFloat, _ := strconv.ParseFloat(minAccountBalance, 10)
	verifyMessageSignatureBool, _ := strconv.ParseBool(verifyMessageSignature)
	verifyAccountBalanceBool, _ := strconv.ParseBool(verifyAccountBalance)
	verifyDataSizeBool, _ := strconv.ParseBool(verifyDataSize)

	config := &relay.RelayServerConfig{
		IdleTimeoutTime:        idleTimeoutTimeInt,
		IdleDisconnectInterval: idleDisconnectIntervalInt,
		MessageDriftTime:       messageDriftTimeInt,
		MinAccountBalance:      float64(minAccountBalanceFloat),
		MaxDataSize:            maxDataSizeInt,
		VerifyMessageSignature: verifyMessageSignatureBool,
		VerifyAccountBalance:   verifyAccountBalanceBool,
		VerifyDataSize:         verifyDataSizeBool,
		MessageRateLimit:       messageRateLimitInt,
	}
	...
	grpc.StartRelayServer(portInt, metricsPortInt, ethClient, config, logger)
}
